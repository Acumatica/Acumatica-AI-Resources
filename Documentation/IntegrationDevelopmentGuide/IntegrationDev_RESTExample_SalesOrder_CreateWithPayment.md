# Create a Sales Order with a Credit Card Payment {#_e7d7b4a4-4609-4a68-ac74-5ab9006d80c5 .concept}

If you are using the contract-based REST API to integrate Acumatica ERP with an external system, this external system can create sales orders with credit card payments in a single API call. For details about the creation of sales orders, see [Reserving Payments for Sales Orders](../UserGuide/SO__con_Payment_Reservation.md).

**Important:** A sales order is created with a payment in one transaction. If a payment cannot be created, the sales order will not be created either.
