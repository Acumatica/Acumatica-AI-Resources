# Automated Synchronization: To Activate a Predefined Business Event {#_58442c4a-eb32-4acd-9cdc-9722ba6e1978 .task}
...
1.  On the [Shopify Stores](BC_20_10_10.md) \(BC201010\) form, specify the store administrator who should receive email notifications about aborted synchronization records.
2.  On the [Business Events](SM_30_20_50.md) \(SM302050\) form, activate the predefined business event that will trigger the sending of an email when the system fails to process a sales order synchronization record more than the maximum number of synchronization attempts allowed.
3.  On the [Email Templates](SM_20_40_03.md) \(SM204003\) form, review the notification template that is used for notifications about aborted sales orders.
...
1.  Make sure that the following prerequisites have been met:
    1.  The Shopify store has been created and configured, as described in [Initial Configuration: To Set Up a Shopify Store](Commerce_SP_Initial_Configuration_To_Set_Up_a_Shopify_Store.md).
    2.  The connection to the Shopify store has been established and the initial configuration has been performed, as described in [Initial Configuration: To Configure the Store Connection](Commerce_SP_Initial_Configuration_Implem_Activity.md).
